...
Dictionary<string, List<string>> ParseHtml(string html)
{
    ...
    var nodeLinks = htmlDoc.DocumentNode.Descendants("div")
...
void WriteToCsv(Dictionary<string, List<string>> mainDictData)
{
    var csv = new StringBuilder();
    List<string> DataFeederSize = mainDictData["titlelist"];
    ...
    {
        var csvFeeder = string.Format("{0},{1},{2}", mainDictData["titlelist"][i], mainDictData["ratinglist"][i], mainDictData["userratinglist"][i]);
        csv.AppendLine(csvFeeder);
        var moviemaster = new MovieMaster { MovieName = mainDictData["titlelist"][i], Userrating = mainDictData["ratinglist"][i], Rating = mainDictData["userratinglist"][i] };
        _context.MovieMaster.Add(moviemaster);
    }
    _context.SaveChanges();
    System.IO.File.AppendAllText("MovieMaster.csv", csv.ToString());
}
